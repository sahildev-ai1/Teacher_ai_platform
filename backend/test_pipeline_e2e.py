"""
Full pipeline smoke test using a mocked LLM (`llm_client.chat`) and mocked
embeddings (`vector_store.embed_texts`) since neither Ollama Cloud nor the
Hugging Face model hub are reachable from this sandbox's network. Everything
else -- FastAPI routing, SQLAlchemy models, the stage state machine, schema
